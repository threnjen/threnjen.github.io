...
model.fit(X_train_val, y_train_val) # Right here is where the model is created - with the fit method. After this line executes, we have a completed model. We could stop right here and move on to predictions, but we want to know some scores first.

model.score(X_train_val, y_train_val) # The R^2 score for our model, if it is simply trained on all of the X_train_val set and then comparing its predictions on the y_train_val actual values
```
